...
product UI must come from that demo, use the dark theme, and leave Settings open
in the desktop capture so the available options remain visible. Image generation
may be used only for the non-product backdrop. The opt-in capture script writes
candidates under `docs/assets/` using Playwright Chromium and `ffmpeg`. On a fresh machine, run
